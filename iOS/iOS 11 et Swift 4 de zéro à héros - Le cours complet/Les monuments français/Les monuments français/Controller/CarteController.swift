...
//
//  Created by Matthieu PASSEREL on 05/03/2018.
...
class CarteController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        carte.register(AnnotationView.self, forAnnotationViewWithReuseIdentifier: MKMapViewDefaultAnnotationViewReuseIdentifier)
        miseEnPlace()
            ...
            if let latitudeString = monument.latitude, let longitudeString = monument.longitude {
                if let latitude = Double(latitudeString), let longitude = Double(longitudeString) {
                    ...
                    MonGeoCoder.obtenir.adresseDepuis(location, completion: { (adresse, erreur) -> (Void) in
                        ...
                        let monAnnotation = MonAnnotation(titre: titre, adresse: monAdresse, coordonnees: coordonnes)
